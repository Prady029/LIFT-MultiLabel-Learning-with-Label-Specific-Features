...
class LIFTTransformer(BaseEstimator, TransformerMixin):
    ...
    def fit(self, X, Y):
        ...
        Args:
            ...
            Y (ndarray): Binary label matrix (n_samples, n_labels)
        """
        n_labels = Y.shape[1]
        self.cluster_info_ = []
    ...
    def transform(self, X):
        ...
        Args:
            X (ndarray): Feature matrix (n_samples, n_features)
        Returns:
            ndarray: Transformed feature matrix
        """
        if self.cluster_info_ is None:
            raise ValueError("LIFTTransformer must be fitted before calling transform().")
...
if __name__ == "__main__":
    ...
    # Initialize and fit LIFT transformer
    lift = LIFTTransformer(k=3, random_state=42)
    X_train_lift = lift.fit_transform(X_train, Y_train)
    X_test_lift = lift.transform(X_test)
